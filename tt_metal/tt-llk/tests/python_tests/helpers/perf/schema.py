...
def _load_perf_metrics_common():
    """Load the shared metric-formula module, tools/tracy/perf_metrics_common.py in the metal tree.

    Located the way counters.py finds hw_counters.h (walk up to the surrounding tt-metal checkout)
    and loaded by file path, so using it does not import the tracy package.
    """
    ...
    for parent in Path(__file__).resolve().parents:
        ...
        if candidate.is_file():
            spec = importlib.util.spec_from_file_location("perf_metrics_common", candidate)
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
            return module
    raise RuntimeError("Could not locate tools/tracy/perf_metrics_common.py above this file")
