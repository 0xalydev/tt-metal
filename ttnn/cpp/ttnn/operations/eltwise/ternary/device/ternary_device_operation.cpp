...
namespace {
// Accessor radix is baked into common_runtime_args at build; must be in key for sharded reuse (#54235).
...
// Prefer Buffer's spec (set_page_size/set_shard_spec reset it while TensorSpec keeps stale copy).
...
// Shape-blind key -- per-core strides re-derive in override_runtime_arguments (mirrors binary_ng).
ttsl::hash::hash_t TernaryDeviceOperation::compute_program_hash(
    ...
    const auto a_shape_in_pages = sharded_tensor_shape_in_pages(input_a);

    ttsl::hash::hash_t hash = tt::tt_metal::operation::hash_operation<TernaryDeviceOperation>(
        args, input_a.dtype(), input_a.memory_config(), a_h, a_w, a_shape_in_pages);
    ...
    if (variant == TernaryVariant::TTT) {
        ...
        hash = tt::tt_metal::operation::hash_operation<TernaryDeviceOperation>(
            ...
            shard_volumes,
            a_shape_in_pages,
            sharded_tensor_shape_in_pages(*input_b),
            sharded_tensor_shape_in_pages(*input_c),
