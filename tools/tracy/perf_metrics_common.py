...
Single source of the derived metrics: the Tracy tool (tracy/perf_counter_analysis.py) and the tt-llk
test harness (tt_metal/tt-llk/tests/python_tests/helpers/metrics.py) each adapt their counter data
to the small CounterView interface below and call compute_metrics(view).

Two metric families, split by key suffix:
  - `*_pct`: bounded percentages (0-100%, value already x100), numerator a subset of its denominator.
  - `*_ratio`: unbounded raw ratios that may exceed 1.0 by design (cross-domain numerator/denominator
    or overlapping sums); never clamped, each explained at its definition.

The counter inventory is per-arch (hw_counters.h), so compute_metrics is the arch UNION and gates
every arch-divergent metric on CounterView.has(): absent counters yield None (N/A), never a fake 0.
"""
...
def perf_counter_type_names(hpp_path=None) -> dict:
    ...
    counter_type field), so it can't drift from the C++ enum. Raises when the header is out of reach,
    e.g. when tracy runs from an installed wheel outside the source tree.
    """
    if hpp_path is None:
...
def mean_port_util(v: "CounterView", bank: str, names, cycles: float) -> "float | None":
    """Mean per-port busy/ref utilization over a client group's PRESENT ports, bounded 0-1.

    Absent ports are excluded from the divisor rather than counted as 0, so the value stays a true
    utilization instead of a sum that scales with port count. None if no port is present.
    """
